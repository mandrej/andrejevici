...
const readExif = async (url: string): Promise<ExifResult | null> => {
  ...
  if (has(exif, 'ApertureValue')) result.aperture = parseFloat(exif.ApertureValue.description)
  ...
  if (has(exif, 'FocalLength')) result.focal_length = parseInt(exif.FocalLength.description)
  if (has(exif, 'ISOSpeedRatings')) result.iso = parseInt(exif.ISOSpeedRatings.description)
  if (has(exif, 'Flash'))
    result.flash = exif.Flash.description.startsWith('Flash did not') ? false : true
