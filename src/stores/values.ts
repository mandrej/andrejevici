...
const buildCounters = async (): Promise<ValuesState["values"]> => {
  ...
  const photoSnapshot = await getDocs(
    query(photoCollection, orderBy("date", "desc")),
  );
  ...
  actions: {
    ...
    async readValues(): Promise<void> {
      ...
      querySnapshot.forEach((d) => {
        ...
        this.values[obj.field as keyof ValuesState["values"]][obj.value] =
          obj.count;
      });
    },
    ...
    async batchUpdateCounters(
      ...
      for (const key of toAdd) {
        ...
        if (import.meta.env.DEV) console.log("increase", key, newCount);
      }
