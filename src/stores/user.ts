  ...
  actions: {
    ...
    async storeUser(user: User): Promise<void> {
      ...
      if (userSnap.exists()) {
        ...
        const lastLogin =
          data.timestamp instanceof Timestamp ? data.timestamp.toMillis() : 0;
        ...
        if (permission === "granted") {
          ...
          if (token) {
            ...
            notify({
              ...
              multiLine: true,
              message:
                "Unable to retrieve notification token. Please try again.",
            });
          }
          ...
          notify({
            type: "warning",
            message:
              "Notifications denied. You can enable them later in browser settings.",
          });
        }
      } catch (err) {
     ...
     */
    async updateUser(
      user: UsersAndDevices,
      field: keyof UsersAndDevices,
    ): Promise<void> {
      ...
      try {
        ...
        notify({
          message: `Updated ${String(field)} to ${value}`,
          icon: "check",
        });
      } catch (err) {
        notify({
